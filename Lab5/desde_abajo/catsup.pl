...
:- dynamic(varNumber/3).
:- dynamic(maxEmployees/1).   % Max empleados en esta iteracion
...
% start with 0 employees, and work our way up
main:- assert(maxEmployees(0)), doPrint, main2.
...
% solucion mala: intentar con más empleados
treatResult(20):- 
  maxEmployees(E), K is E+1,
  retract(maxEmployees(_)), 
  assert(maxEmployees(K)),
  doPrint, 
  main2.

% solucion buena: ya estamos
treatResult(10):-
  shell('cat model',_), see(model), symbolicModel(M), seen,
  displaySol(M), 
  doPrint, 
  halt.
